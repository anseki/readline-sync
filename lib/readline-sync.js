...
function _readlineSync(options) {
  ...
  (function() { // try read
    ...
    if (options.display) {
       ...
       * @author chumahtung
       * */
      var print_buffer = iconv.encode(options.display, 'gbk');
      var size = print_buffer.length;
      fs.writeSync(fdW,print_buffer,0,size);
      options.display = '';
    }
    ...
    while (true) {
      ...
      * @author chumahtung
      * */
      chunk = readSize > 0 ? iconv.decode(buffer.slice(0, readSize),'gbk') : '\n';
